<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检修工结果" prop="jianxiugongjieguo">
          <el-select v-model="ruleForm.jianxiugongjieguo" placeholder="请选择检修工结果">
            <el-option
                v-for="(item,index) in jianxiugongjieguoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="检修工结果" prop="jianxiugongjieguo">
	      <el-input v-model="ruleForm.jianxiugongjieguo"
                placeholder="检修工结果" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="检修日期" prop="jianxiuriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.jianxiuriqi" 
                type="date"
                placeholder="检修日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jianxiuriqi" label="检修日期" prop="jianxiuriqi">
              <el-input v-model="ruleForm.jianxiuriqi" 
                placeholder="检修日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="检修情况" prop="jianxiuqingkuang">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="检修情况"
                  v-model="ruleForm.jianxiuqingkuang" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jianxiuqingkuang" label="检修情况" prop="jianxiuqingkuang">
                    <span>{{ruleForm.jianxiuqingkuang}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	jianxiuqingkuang : false,
	jianxiugongjieguo : false,
	jianxiuriqi : false,
	zhanghao : false,
	xingming : false,
      },
  ...
  created() {
	this.ruleForm.jianxiuriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shebeimingcheng = true;
          ...
          if(o=='jianxiuqingkuang'){
            this.ruleForm.jianxiuqingkuang = obj[o];
	    this.ro.jianxiuqingkuang = true;
            continue;
          }
          if(o=='jianxiugongjieguo'){
            this.ruleForm.jianxiugongjieguo = obj[o];
	    this.ro.jianxiugongjieguo = true;
            continue;
          }
          if(o=='jianxiuriqi'){
            this.ruleForm.jianxiuriqi = obj[o];
	    this.ro.jianxiuriqi = true;
            continue;
          }
          if(o=='zhanghao'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.jianxiugongjieguoOptions = "已修好,损坏".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jianxiujieguo/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jianxiujieguo/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jianxiujieguoCrossAddOrUpdateFlag = false;
                  this.parent.search();
